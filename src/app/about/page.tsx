      ...
      <div className="relative z-10">
        ...
        <header className="sticky top-0 z-50">
          ...
          <nav className="hidden items-center space-x-8 md:flex">
            <a href="#story" className="transition-colors hover:text-secondary" style={{color: 'var(--text-primary)'}}>Our Story</a>
        ...
        <section id="story" className="py-20">
          <div className="container mx-auto px-4">
            <div className="mx-auto grid max-w-6xl grid-cols-1 items-center gap-12 lg:grid-cols-2">
              <div>
                <h2 className="mb-6 text-4xl font-bold" style={{color: 'var(--text-primary)'}}>Why We Chose the Tree</h2>
                <div className="space-y-6 text-lg leading-relaxed" style={{color: 'var(--text-primary)'}}>
                  <p>
                    <Quote className="mr-2 inline h-6 w-6" style={{color: 'var(--text-secondary)'}} />
                    Just like a mighty oak that has weathered countless storms, your business deserves to be sold with the same strength and reliability.
                  </p>
                  <p>
                ...
                <div className="mt-8 flex items-center space-x-6">
                  <div className="text-center">
                    <div className="text-3xl font-bold" style={{color: 'var(--text-primary)'}}>100%</div>
                    <div className="text-sm" style={{color: 'var(--text-primary)'}}>Verification Rate</div>
                  </div>
                  <div className="text-center">
                    <div className="text-3xl font-bold" style={{color: 'var(--text-primary)'}}>0</div>
                    <div className="text-sm" style={{color: 'var(--text-primary)'}}>Reported Scams</div>
                  </div>
                  <div className="text-center">
                    <div className="text-3xl font-bold" style={{color: 'var(--text-primary)'}}>10min</div>
                    <div className="text-sm" style={{color: 'var(--text-primary)'}}>Average Setup</div>
                  </div>
                </div>
              </div>
              
              <div className="relative">
                <div className="relative flex h-80 w-full items-center justify-center overflow-hidden rounded-3xl" style={{
                  background: 'linear-gradient(to bottom right, var(--primary)/20, var(--secondary)/20)'
                }}>
                  <div className="absolute inset-0" style={{
                    background: 'linear-gradient(to top, var(--primary)/10, transparent)'
                  }}></div>
                  {/* Floating elements */}
                  <div className="absolute left-8 top-8 h-4 w-4 animate-pulse rounded-full" style={{backgroundColor: 'var(--text-secondary)/30'}}></div>
                  <div className="absolute bottom-12 right-12 h-6 w-6 animate-pulse rounded-full" style={{backgroundColor: 'var(--text-primary)/40', animationDelay: '1s'}}></div>
                  <div className="absolute right-8 top-1/2 h-3 w-3 animate-pulse rounded-full" style={{backgroundColor: 'var(--text-primary)/40', animationDelay: '2s'}}></div>
                </div>
              </div>
            </div>
          </div>
        ...
        <section className="py-20">
          <div className="container mx-auto px-4">
            <div className="mb-16 text-center">
              <h2 className="mb-4 text-4xl font-bold" style={{color: 'var(--text-primary)'}}>Stories from Our Grove</h2>
              <p className="mx-auto max-w-2xl text-xl" style={{color: 'var(--text-primary)'}}>
                Real business owners who found their perfect match through Grounding
              </p>
            </div>
            ...
            <div className="mx-auto grid max-w-6xl grid-cols-1 gap-8 md:grid-cols-3">
              {[
                ...
                },
                {
                  ...
                  quote: "The verification process gave me complete peace of mind. I knew every inquiry was from a serious, qualified buyer.",
                  rating: 5
                },
                {
                  name: "Jennifer Adams",
                  business: "Retail Chain",
                  quote: "From listing to sale in 6 weeks. The trust-first approach made all the difference in finding the right successor.",
                  rating: 5
                }
              ].map((story, index) => (
                <Card key={index} className="p-6 transition-all duration-300 hover:shadow-xl hover:scale-105" style={{
                  borderColor: 'var(--border-light)/30',
                  '--tw-hover-border-opacity': '0.3',
                } as React.CSSProperties}>
                  <div className="mb-4 flex items-center">
                    {[...Array(story.rating)].map((_, i) => (
                      <Star key={i} className="h-5 w-5 fill-current" style={{color: 'var(--text-secondary)'}} />
                    ))}
                  </div>
                  ...
                  <div className="flex items-center space-x-3">
                    ...
                    <div>
                      <div className="font-semibold" style={{color: 'var(--text-primary)'}}>{story.name}</div>
                      <div className="text-sm" style={{color: 'var(--text-primary)'}}>{story.business}</div>
                    </div>
                  </div>
                </Card>
              ))}
